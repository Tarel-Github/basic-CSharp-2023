...
namespace wf07_myexplorer
{
    public partial class FrmExplorer : Form
    {
        ...
        private void FrmExplorer_Load(object sender, EventArgs e)
        {
            ...
            // 현재 컴퓨터에 존재하는 드라이브 정보를 전부다 검색해서
            DriveInfo[] drives = DriveInfo.GetDrives();

            // 트리뷰에 전부 추가
            foreach (DriveInfo drive in drives) 
            {
                // 실제 존재하는 하드드라이브만 가져오도록 하는 조건문
        ...
        private void FillNodes(TreeNode curNode)
        {
            ...
            {
                MessageBox.Show("오류발생! 오류발생!", "오류", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        //sender는 자기자신 객체 내용, e 이벤트와 관련된 속성포함
        /// <summary>
        /// 트리뷰 노드 확장하기 전 이벤트
        /// </summary>
        ...
        private void TrvDrive_BeforeExpand(object sender, TreeViewCancelEventArgs e)
        {
            ...
            {
                e.Node.Nodes.Clear();
                e.Node.ImageIndex = 1;          // 폴더
                e.Node.SelectedImageIndex = 2;  // 열린 폴더
        ...
        {
            if (e.KeyChar == 13)    // 13번은 KeyBoard 엔터키를 누른경우
            {
                try
                {
